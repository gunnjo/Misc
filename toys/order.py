...
def doalgo( image):
    ...
    scaleshow("BG", t[:,:,1])    
    t = cv2.cvtColor( image, cv.CV_BayerGB2RGB )
    scaleshow("GB", t[:,:,1])    
    t = cv2.cvtColor( image, cv.CV_BayerRG2RGB )
    scaleshow("RG", t[:,:,1])    
    t = cv2.cvtColor( image, cv.CV_BayerGR2RGB )
    scaleshow("GR", t[:,:,1])    
    t = cv2.cvtColor( image, cv.CV_BayerBG2BGR )
    scaleshow("BG2", t[:,:,1])    
    t = cv2.cvtColor( image, cv.CV_BayerGB2BGR )
    scaleshow("GB2", t[:,:,1])    
    t = cv2.cvtColor( image, cv.CV_BayerRG2BGR )
    scaleshow("RG2", t[:,:,1])    
    t = cv2.cvtColor( image, cv.CV_BayerGR2BGR )
    scaleshow("GR2", t[:,:,1])    
