...
def xxxx(silh, dst, timestamp, comp_rect, color, magnitude):
    global mhi
    global orient
    global mask
    ...
    if( count >= comp_rect[2] * comp_rect[3] * 0.05 ):
        ...
        cv.Line( dst, center, ( cv.Round( center[0] + magnitude*cos(angle*cv.CV_PI/180)),
            cv.Round( center[1] - magnitude*sin(angle*cv.CV_PI/180))), color, 3, cv.CV_AA, 0 )
...
def update_mhi( img, dst, diff_threshold ):
    ...
    idx1 = last
    if not mhi or mhi[0] != size[0] or mhi[1] != size[1]: 
        for i in range( N ):
    ...
    xxxx( silh, dst, timestamp, ( 0, 0, size[0], size[1] ), cv.CV_RGB(255,255,255), 100.)
    for i in seq:
        comp_rect = i[1] 
        if( comp_rect[2] + comp_rect[3] < (size[0]+size[1])*.15 ): # reject very small components
            continue
        xxxx( silh, dst, timestamp, comp_rect, cv.CV_RGB(255,0,0), 30.)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if( image ):
            if( not motion ):
                    motion = cv.CreateImage( cv.GetSize(image), 8, 3 )
                    cv.Zero( motion )
...
#vim: set ts:4 sw:4 ai et syntax
